...
function isIsoscelesTriangle(a, b, c) {
  if (a === b) {
    if (a + b >= c && c !== 0) return true;
  }
  if (a === c) {
    if (a + c >= b && b !== 0) return true;
  }
  if (b === c) {
    if (b + c >= a && a !== 0) return true;
  }
...
function isPalindrome(str) {
  const lengthString = str.length - 1;

  for (let id = 0; id < str.length; id += 1) {
    const elmLeft = str[id];
    const elmRight = str[lengthString - id];
    if (elmLeft !== elmRight) return false;
  }
  return true;
}
...
function getIndexOf(str, letter) {
  for (let id = 0; id < str.length; id += 1) {
    const element = str[id];
    if (element === letter) return id;
  }
  return -1;
}
...
function isContainNumber(num, digit) {
  let lastNumber = Math.floor(num % 10);
  let currentNumber = num;
  while (currentNumber !== 0) {
    lastNumber = Math.floor(currentNumber % 10);
    currentNumber = Math.floor(currentNumber / 10);
    if (lastNumber === digit) return true;
  }
  return false;
}
